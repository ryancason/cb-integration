...
'''
    ...
    _quickscan_get = (
        'SELECT md5sum FROM binary_data WHERE ( ( state = 0 AND quick_scan_done = 0 ) OR ((julianday(\'?\') - julianday(last_modified)) > ?) ) AND retry_count < ? LIMIT 1'
    )
    ...
    def get(self, quick_scan=False, sleep_wait=True):
        ...
        with self._get_conn() as conn:
            ...
            while keep_pooling:
                ...
                if quick_scan:
                    cursor = conn.execute(self._quickscan_get, (datetime.datetime.now(), self.num_days_before_rescan, self.max_retry_count))
                else:
                    cursor = conn.execute(self._popleft_get, (datetime.datetime.now(), self.num_days_before_rescan, datetime.datetime.now() ,self.max_retry_count))
